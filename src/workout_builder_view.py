...
class View:
    def __init__(self, master):
        ...
        self._lbl_header = tk.Label(master=self._frm_header,
                                    text='Workout Name: ', width=12, anchor='w')
    ...
    def add_block(self, quantity=1, exercises=0):
        for x in range(0, quantity):
            self._exercise_blocks.append(ExerciseBlock(self._frm,
                                         len(self._exercise_blocks) + 1))
            if exercises:
                self._exercise_blocks[len(self._exercise_blocks) - 1].add_exercise(exercises)


class ExerciseBlock:
    labels = [('classification', 22), ('exercise name', 37), ('sets', 7),
              ('reps', 7), ('intensity', 7), ('video link', 22)]
    ...
    def __init__(self, master, block_num):
        ...
        self._lbl_block_name = tk.Label(master=self._frm_header, 
                                        text='Block Name: ', width=12, anchor='w')
        ...
        for i in range(len(ExerciseBlock.labels)):
            lbl = tk.Label(master=self._frm_body_labels, text=ExerciseBlock.labels[i][0],
                           width=ExerciseBlock.labels[i][1], anchor='w')
